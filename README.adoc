...
`docker run --rm --tty --user "$(id -u)" --volume "${PWD}":/documents vshn/asciidoctor-pdf:1.8.1 file.adoc`

Any parameters passed after the command line will be fed directly to Asciidoctor, for example

`docker run --rm --tty --user "$(id -u)" --volume "${PWD}":/documents vshn/asciidoctor-pdf:1.8.1 file.adoc --destination-dir=./_archive`
